...
export class EquationManager2 {
  ...
  create() {
    ...
    for (let theta = 0; theta < 2 * Math.PI ; theta += Math.PI / 32) { // 512 items ---  256*2    128*4    64*8
      ...
      thing.position.x = radius * Math.sin(theta+this.thetaDelta);
      thing.position.z = radius * Math.cos(theta+this.thetaDelta);
      thing.position.y = 60*Math.atan(thing.position.z)* Math.cos(theta+this.thetaDelta) ;
